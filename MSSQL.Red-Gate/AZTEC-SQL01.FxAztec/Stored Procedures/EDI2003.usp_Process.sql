...
CREATE procedure [EDI2003].[usp_Process]
...
set	@ProcName = user_name(objectproperty(@@procid, 'OwnerId')) + '.' + object_name(@@procid)  -- e.g. EDI2003.usp_Test
--- </Error Handling>
...
,   NewDocument
from
	EDI2003.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDI2003.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI2003.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI2003.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDI2003.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2003.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2003.PlanningReleases', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDI2003.ShipScheduleHeaders'
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDI2003.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI2003.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDI2003.PlanningReleases'
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI2003.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI2003.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDI2003.PlanningReleases PR
...
--- <Update rows="*">
set	@TableName = 'EDI2003.PlanningHeaders'
	...
	case
		when exists
			(	select
					*
				from
					EDI2003.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI2003.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI2003.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDI2003.ShipScheduleHeaders fh
...
from
	EDI2003.ShipScheduleHeaders fh
	join EDI2003.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI2003.ShipScheduleAccums fa
	...
	left join EDI2003.ShipScheduleAuthAccums faa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDI2003.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDI2003.PlanningHeaders fh
	join EDI2003.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI2003.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDI2003.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDI2003.PlanningReleases', 'Status', 'Active'))
...
if	@Testing = 0 begin
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDI2003.PlanningHeaders fh
					join EDI2003.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDI2003.ShipScheduleHeaders fh
					join EDI2003.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDI2003.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDI2003.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDI2003.ShipSchedules b
 Join 
	EDI2003.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDI2003.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDI2003.ShipScheduleAccums ssa on 
 	...
 	left join
		EDI2003.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDI2003.PlanningAuthAccums praa on 
...
Begin
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDI2003' 
...
execute
	@ProcReturn = EDI2003.usp_Process
	@TranDT = @TranDT out
