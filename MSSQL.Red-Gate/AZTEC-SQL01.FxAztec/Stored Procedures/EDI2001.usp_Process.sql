...
CREATE procedure [EDI2001].[usp_Process]
...
,   NewDocument
from
	EDI2001.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDI2001.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI2001.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI2001.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDI2001.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2001.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2001.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDI2001.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI2001.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI2001.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI2001.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDI2001.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDI2001.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI2001.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI2001.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDI2001.ShipScheduleHeaders fh
...
from
	EDI2001.ShipScheduleHeaders fh
	join EDI2001.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI2001.ShipScheduleAccums fa
	...
	left join EDI2001.ShipScheduleAuthAccums faa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDI2001.ShipScheduleHeaders', 'Status', 'Active'))
...
,	ModelYear = bo.ModelYear
,	OrderUnit = bo.OrderUnit
,	ReleaseNo = coalesce(nullif(fr.Userdefined1,''),fr.ReleaseNo) --Capture Kautex DO Number into release number column
,	QtyRelease = fr.ReleaseQty
,	StdQtyRelease = fr.ReleaseQty
,	ReferenceAccum = case bo.ReferenceAccum 
												When 'N' 
...
from
	EDI2001.PlanningHeaders fh
	join EDI2001.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI2001.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDI2001.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDI2001.PlanningReleases', 'Status', 'Active'))
...
if	@Testing = 0 begin
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDI2001.PlanningHeaders fh
					join EDI2001.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDI2001.ShipScheduleHeaders fh
					join EDI2001.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDI2001.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDI2001.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDI2001.ShipSchedules b
 Join 
	EDI2001.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDI2001.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDI2001.ShipScheduleAccums ssa on 
 	...
 	left join
		EDI2001.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDI2001.PlanningAuthAccums praa on 
...
Begin
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDI2001' 
...
execute
	@ProcReturn = EDI2001.usp_Process
	@TranDT = @TranDT out
