...
CREATE procedure [EDI2040].[usp_Process]
...
,   NewDocument
from
	EDI2040.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDI2040.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI2040.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI2040.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDI2040.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2040.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2040.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDI2040.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI2040.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI2040.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI2040.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDI2040.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDI2040.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI2040.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI2040.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDI2040.ShipScheduleHeaders fh
...
from
	EDI2040.ShipScheduleHeaders fh
	join EDI2040.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI2040.ShipScheduleAccums fa
	...
	left join EDI2040.ShipScheduleAuthAccums faa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDI2040.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDI2040.PlanningHeaders fh
	join EDI2040.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI2040.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDI2040.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDI2040.PlanningReleases', 'Status', 'Active'))
...
--	,	ReleaseNo = case when rr.ReferenceAccum > rr.RelPost Then 'Accum Increase' else rr.ReleaseNo end
from
	@RawReleases rr
...
if	@Testing = 0 begin
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDI2040.PlanningHeaders fh
					join EDI2040.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDI2040.ShipScheduleHeaders fh
					join EDI2040.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDI2040.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDI2040.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDI2040.ShipSchedules b
 Join 
	EDI2040.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDI2040.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDI2040.ShipScheduleAccums ssa on 
 	...
 	left join
		EDI2040.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDI2040.PlanningAuthAccums praa on 
...
Begin
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDI2040' 
...
execute
	@ProcReturn = EDI2040.usp_Process
	@TranDT = @TranDT out
