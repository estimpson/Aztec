...
create procedure [dbo].[usp_InventoryControl_Transfer_Pallet]
	@Operator varchar(5)
,	@PalletSerial int
,	@Location varchar(10)
...
if not exists
	...
	set	@Result = 60002
	rollback tran @ProcName
	RAISERROR ('Error in procedure %s. Serial %d not found.', 16, 1, @ProcName, @PalletSerial)
	return	@Result
end
...
if exists
	...
	set	@Result = 60001
	rollback tran @ProcName
	RAISERROR ('Error in procedure %s. Pallet %d is already at location %s.', 16, 1, @ProcName, @PalletSerial, @Location)
	return	@Result
end
---	</ArgumentValidation>
