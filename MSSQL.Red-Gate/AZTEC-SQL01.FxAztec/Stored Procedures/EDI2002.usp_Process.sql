...
,   NewDocument
from
	EDI2002.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDI2002.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI2002.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI2002.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDI2002.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2002.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2002.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDI2002.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI2002.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI2002.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI2002.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI2002.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI2002.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI2002.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI2002.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDI2002.PlanningReleases PR
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDI2002.ShipScheduleHeaders fh
...
from
	EDI2002.ShipScheduleHeaders fh
	join EDI2002.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI2002.ShipScheduleAccums fa
	...
	left join EDI2002.ShipScheduleAuthAccums faa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDI2002.ShipScheduleHeaders', 'Status', 'Active'))
...
if	@Testing = 0 begin
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDI2002.PlanningHeaders fh
					join EDI2002.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDI2002.ShipScheduleHeaders fh
					join EDI2002.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDI2002.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDI2002.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDI2002.ShipSchedules b
 Join 
	EDI2002.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDI2002.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDI2002.ShipScheduleAccums ssa on 
 	...
 	left join
		EDI2002.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDI2002.PlanningAuthAccums praa on 
...
Begin
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDI2002' 
...
execute
	@ProcReturn = EDI2002.usp_Process
	@TranDT = @TranDT out
