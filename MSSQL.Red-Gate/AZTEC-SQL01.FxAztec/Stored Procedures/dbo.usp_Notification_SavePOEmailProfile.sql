...
Create procedure [dbo].[usp_Notification_SavePOEmailProfile]
	@PONumber int
...
/*	Insert or update the PO's Email profile. */
--- <Update rows="1">
set	@TableName = 'dbo.Notification_POEmailProfile'
...
set
	ProfileID = @ProfileID
from
	dbo.Notification_POEmailProfile npep
where
	npep.PONumber = @PONumber
...
if	@RowCount != 1 begin
	...
	insert
		dbo.Notification_POEmailProfile
	(	PONumber
	,	ProfileID
	)
	select
...
execute
	dbo.usp_Notification_SavePOEmailProfile
	@PONumber = @PONumber
,	@EmailTo = @EmailTo
,	@EmailCC = @EmailCC
,	@EmailReplyTo = @EmailReplyTo
