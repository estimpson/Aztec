...
CREATE procedure [EDI3020].[usp_Process]
...
,   NewDocument
from
	EDI3020.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDI3020.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI3020.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI3020.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDI3020.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDI3020.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI3020.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDI3020.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI3020.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI3020.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI3020.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDI3020.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDI3020.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI3020.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI3020.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDI3020.ShipScheduleHeaders fh
...
from
	EDI3020.ShipScheduleHeaders fh
	join EDI3020.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI3020.ShipScheduleAccums fa
	...
	left join EDI3020.ShipScheduleAuthAccums faa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDI3020.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDI3020.PlanningHeaders fh
	join EDI3020.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI3020.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDI3020.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDI3020.PlanningReleases', 'Status', 'Active'))
...
if	@Testing = 0 begin
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDI3020.PlanningHeaders fh
					join EDI3020.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDI3020.ShipScheduleHeaders fh
					join EDI3020.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDI3020.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDI3020.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDI3020.ShipSchedules b
 Join 
	EDI3020.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDI3020.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDI3020.ShipScheduleAccums ssa on 
 	...
 	left join
		EDI3020.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDI3020.PlanningAuthAccums praa on 
...
Begin
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDI3020' 
...
execute
	@ProcReturn = EDI3020.usp_Process
	@TranDT = @TranDT out
