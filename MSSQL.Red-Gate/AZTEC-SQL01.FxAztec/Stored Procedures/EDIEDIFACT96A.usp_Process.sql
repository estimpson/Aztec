...
--- <Update rows="*">
set	@TableName = 'EDIEDIFACT96A.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIEDIFACT96A.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIEDIFACT96A.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIEDIFACT96A.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIEDIFACT96A.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIEDIFACT96A.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIEDIFACT96A.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIEDIFACT96A.ShipScheduleHeaders fh
...
from
	EDIEDIFACT96A.ShipScheduleHeaders fh
	join EDIEDIFACT96A.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIEDIFACT96A.ShipScheduleAccums fa
	...
	left join EDIEDIFACT96A.ShipScheduleAuthAccums faa
	...
	join EDIEDIFACT96A.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIEDIFACT96A.PlanningHeaders fh
	join EDIEDIFACT96A.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIEDIFACT96A.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIEDIFACT96A.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningReleases', 'Status', 'Active'))
...
if	@Testing = 0 begin
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIEDIFACT96A.PlanningHeaders fh
					join EDIEDIFACT96A.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIEDIFACT96A.ShipScheduleHeaders fh
					join EDIEDIFACT96A.ShipScheduleSupplemental sss
		...
		*/
		...
		from
			dbo.order_header oh
		join
				EDIEDIFACT96A.blanketOrders bo
		on
		...
		join
		  ...
		  join
				EDIEDIFACT96A.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIEDIFACT96A.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIEDIFACT96A.ShipSchedules b
 Join 
	EDIEDIFACT96A.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
and not exists
( Select 1 from 
		EDIEDIFACT96A.PlanningReleases b
 Join 
	EDIEDIFACT96A.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
	b.ShipToCode = bo.EDIShipToCode
...
from
	@Current862s a
	 Join 
	EDIEDIFACT96A.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIEDIFACT96A.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIEDIFACT96A.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIEDIFACT96A.PlanningAccums pra on 
 	...
 	left join
		EDIEDIFACT96A.PlanningAuthAccums praa on 
...
Begin
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDIEDIFACT96A' 
...
FROM
	@Current830s a
JOIN
		EDIEDIFACT96A.PlanningReleases c
ON			c.RawDocumentGUID = a.RawDocumentGUID AND
				a.CustomerPart = c.CustomerPart AND
				a.ShipToCode =c.ShipToCode AND
				COALESCE(a.customerPO,'') = COALESCE(c.CustomerPO,'') AND
...
execute
	@ProcReturn = EDIEDIFACT96A.usp_Process
	@TranDT = @TranDT out
