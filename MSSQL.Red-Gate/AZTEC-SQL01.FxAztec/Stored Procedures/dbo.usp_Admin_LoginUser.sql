...
set	@CallProcName = 'dbo.xp_Security_ValidateLicense'
execute
	@ProcReturn = dbo.xp_Security_ValidateLicense
...
if	@ProcResult = 2 begin
	/*	License valid. */
	set @Result = 0
end
if	@ProcResult = 1 begin
	/*	License invalid, grace period in effect. */
	set @Result = 100
end
if	@ProcResult = 0 begin
	/* Exception thrown or general error, could not determine license validity */
	raiserror ('Error encountered in %s.  Message: %s', 16, 1, @ProcName, @Message)
	rollback tran @ProcName
	return
end
if	@ProcResult = -1 begin
	/*	License expired and grace period has ended. */
	set @Result = -1
	raiserror ('Error encountered in %s.  Message: %s', 16, 1, @ProcName, @Message)
	rollback tran @ProcName
	return
...
if (@TranCount = 0) commit tran @ProcName 
