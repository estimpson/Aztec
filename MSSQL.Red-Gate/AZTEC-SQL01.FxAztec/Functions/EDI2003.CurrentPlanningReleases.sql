...
CREATE FUNCTION [EDI2003].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI2003.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI2003.PlanningHeaders ph
				JOIN EDI2003.PlanningReleases pr
		...
		JOIN EDI2003.PlanningHeaders ph
			JOIN EDI2003.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI2003.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
