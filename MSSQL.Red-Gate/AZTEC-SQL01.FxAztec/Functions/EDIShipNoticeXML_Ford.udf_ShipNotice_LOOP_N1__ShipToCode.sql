...
CREATE FUNCTION [EDIShipNoticeXML_Ford].[udf_ShipNotice_LOOP_N1__ShipToCode]
(	@ShipperID INT
)
RETURNS XML
AS
		...
		(	SELECT
			...
			,	(	SELECT
					...
					,	(	SELECT
							...
							,	[DE!1!name]='NAME'
							,	[DE!1!type]='AN'
							--,	[DE!1]= COALESCE(d.name, 'Ford Motor')
							FOR XML EXPLICIT, TYPE
			 			)
					/*	DE 0066*/
					,	(	SELECT
					...
					,	(	SELECT
							...
							,	[DE!1!type]='AN'
							,	[DE!1]= COALESCE(es.EDIShipToID, es.parent_destination, es.destination, '240797746')
							FOR XML EXPLICIT, TYPE
			 			)
					FROM Shipper s 
					JOIN edi_setups es ON es.destination = s.destination
					JOIN dbo.destination d ON d.destination = s.destination
...
--- </Body>
-- Select [EDIShipNoticeXML_Ford].[udf_ShipNotice_LOOP_N1__ShipToCode] (1062979)
---	<Return>
	RETURN
		@outputXML
END
