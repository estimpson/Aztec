...
CREATE FUNCTION [EDI3060].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ssh.Status = 0 --(select dbo.udf_StatusValue('EDI3060.ShipScheduleHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				...
				JOIN EDI3060.ShipSchedules ss
					ON ss.RawDocumentGUID = ssh.RawDocumentGUID
			WHERE
				ssh.Status IN
				(	0 --(select dbo.udf_StatusValue('EDI3060.ShipScheduleHeaders', 'Status', 'New'))
				,	1 --(select dbo.udf_StatusValue('EDI3060.ShipScheduleHeaders', 'Status', 'Active'))
				)
			GROUP BY
				ss.ShipToCode
		...
		JOIN EDI3060.ShipScheduleHeaders ssh
			...
			LEFT JOIN
				EDI3060.BlanketOrders bo ON bo.EDIShipToCode = ss.ShipToCode
