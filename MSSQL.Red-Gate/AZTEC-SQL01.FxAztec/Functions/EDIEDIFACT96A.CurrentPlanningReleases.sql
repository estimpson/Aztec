...
CREATE FUNCTION [EDIEDIFACT96A].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIEDIFACT96A.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIEDIFACT96A.PlanningHeaders ph
				JOIN EDIEDIFACT96A.PlanningReleases pr
		...
		JOIN EDIEDIFACT96A.PlanningHeaders ph
			JOIN EDIEDIFACT96A.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIEDIFACT96A.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
