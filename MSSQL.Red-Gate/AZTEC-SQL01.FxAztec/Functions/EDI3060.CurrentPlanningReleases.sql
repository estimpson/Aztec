...
CREATE FUNCTION [EDI3060].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI3060.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI3060.PlanningHeaders ph
				JOIN EDI3060.PlanningReleases pr
		...
		JOIN EDI3060.PlanningHeaders ph
			JOIN EDI3060.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI3060.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
