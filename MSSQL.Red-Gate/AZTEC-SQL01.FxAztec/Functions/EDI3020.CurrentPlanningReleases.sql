...
CREATE FUNCTION [EDI3020].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI3020.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI3020.PlanningHeaders ph
				JOIN EDI3020.PlanningReleases pr
		...
		JOIN EDI3020.PlanningHeaders ph
			JOIN EDI3020.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI3020.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
