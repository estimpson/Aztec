...
CREATE FUNCTION [EDI2001].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI2001.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI2001.PlanningHeaders ph
				JOIN EDI2001.PlanningReleases pr
		...
		JOIN EDI2001.PlanningHeaders ph
			JOIN EDI2001.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI2001.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
