...
CREATE FUNCTION [EDI2002].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI2002.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI2002.PlanningHeaders ph
				JOIN EDI2002.PlanningReleases pr
		...
		JOIN EDI2002.PlanningHeaders ph
			JOIN EDI2002.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI2002.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
