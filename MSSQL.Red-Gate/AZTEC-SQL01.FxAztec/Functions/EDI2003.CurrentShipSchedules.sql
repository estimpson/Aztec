...
CREATE FUNCTION [EDI2003].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ssh.Status = 0 --(select dbo.udf_StatusValue('EDI2003.ShipScheduleHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				...
				JOIN EDI2003.ShipSchedules ss
					ON ss.RawDocumentGUID = ssh.RawDocumentGUID
			WHERE
				ssh.Status IN
				(	0 --(select dbo.udf_StatusValue('EDI2003.ShipScheduleHeaders', 'Status', 'New'))
				,	1 --(select dbo.udf_StatusValue('EDI2003.ShipScheduleHeaders', 'Status', 'Active'))
				)
			GROUP BY
				ss.ShipToCode
		...
		JOIN EDI2003.ShipScheduleHeaders ssh
			JOIN EDI2003.ShipSchedules ss
			ON ss.RawDocumentGUID = ssh.RawDocumentGUID
			...
			LEFT JOIN
				EDI2003.BlanketOrders bo ON bo.EDIShipToCode = ss.ShipToCode
