...
CREATE FUNCTION [EDI2040].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI2040.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI2040.PlanningHeaders ph
				JOIN EDI2040.PlanningReleases pr
		...
		JOIN EDI2040.PlanningHeaders ph
			JOIN EDI2040.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI2040.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
