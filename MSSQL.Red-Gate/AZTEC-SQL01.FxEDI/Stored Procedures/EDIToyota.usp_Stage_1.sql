...
--Begin Transaction
--Toyota does not send Accums - Commented Section for efficiency purposes
/*
declare
...
/*			- prepare Release Plans Supplemental.*/
	...
	select
		RawDocumentGUID
	,	ReleaseNo = coalesce(nullif(ed.Data.value('(/TRN-830/SEG-BFR/DE[@code="0328"])[1]', 'varchar(30)'),''), ed.Data.value('(/TRN-830/SEG-BFR/DE[@code="0127"])[1]', 'varchar(30)'))
	,	ShipToCode =  coalesce(ed.Data.value('(/TRN-830/LOOP-N1/SEG-N1 [DE[.="MI"][@code="0098"]]/DE[@code="0093"])[1]', 'varchar(50)'),EDIData.Releases.value('(./SEG-REF [DE[.="DK"][@code="0128"]]/DE[@code="0127"])[1]', 'varchar(50)'), ed.Data.value('/*[1]/TRN-INFO[1]/@trading_partner', 'varchar(50)'))
...
/*			- prepare Release Plans Accums.*/
	--Begin Transaction
	--Commented Accums Sections Toyota does not use Accums
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxAztec.EDIToyota.StagingShipScheduleAccums'

	insert  FxAztec.EDIToyota.StagingShipScheduleAccums
    (	RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxAztec.EDIToyota.StagingShipScheduleAuthAccums'

	insert
		FxAztec.EDIToyota.StagingShipScheduleAuthAccums
	(	
		RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxAztec.EDIToyota.StagingPlanningAccums'

	insert
		FxAztec.EDIToyota.StagingPlanningAccums
	 (	RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
	,	ConsigneeCode
...
if	exists
	...
	--- <Insert rows="*">
	set	@TableName = 'FxAztec.EDIToyota.StagingPlanningAuthAccums'

	insert
		FxAztec.EDIToyota.StagingPlanningAuthAccums
	(	
		RawDocumentGUID
	,	ReleaseNo
	,	ShipToCode
...
set	@Result = 100
	raiserror ('Chrysler documents already in process.  Use EDIToyota.usp_ClearQueue to clear the queue if necessary.', 16, 1)
	RETURN
