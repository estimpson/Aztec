...
create procedure [EDI3060].[usp_StageReleasePlans]
...
begin
	...
	begin try
		...
		begin
			...
			update
				rph
			set	ReleaseNo = coalesce
					(	nullif(Data.value('(/TRN-830/SEG-BFR/DE[@code="0328"])[1]', 'varchar(30)'), '')
					,	Data.value('(/TRN-830/SEG-BFR/DE[@code="0127"])[1]', 'varchar(30)')
					)
			,	DocumentDT = coalesce
					(	Data.value('(/TRN-830/SEG-BFR/DE[@code="0373"])[2]', 'datetime')
					,	Data.value('(/TRN-830/SEG-BFR/DE[@code="0373"])[1]', 'datetime')
					)
			from
			...
			update
				rp
			set	CustomerPart = Data.value ('(for $a in LOOP-LIN/SEG-LIN/DE[@code="0235"] where $a="BP" return $a/../DE[. >> $a][@code="0234"][1])[1]', 'varchar(30)')
			,	CustomerPO = Data.value('(for $a in LOOP-LIN/SEG-LIN/DE[@code="0235"] where $a="PO" return $a/../DE[. >> $a][@code="0234"][1])[1]', 'varchar(30)')
			,	CustomerPOLine = Data.value('(for $a in LOOP-LIN/SEG-LIN/DE[@code="0235"] where $a="PL" return $a/../DE[. >> $a][@code="0234"][1])[1]', 'varchar(30)')
			from
				@ReleasePlans rp
...
execute
	@ProcReturn = EDI3060.usp_StageReleasePlans
	@TranDT = @TranDT out
