...
/*
Create Procedure.Fx.EDIToyota.usp_Process.sql
*/

--use Fx
--go

if	objectproperty(object_id('EDIToyota.usp_Process'), 'IsProcedure') = 1 begin
	drop procedure EDIToyota.usp_Process
end
go
...
update
	ss
set
	Status = 1 --(select dbo.udf_StatusValue('EDIToyota.ShipSchedules', 'Status', 'Active'))
from
...
update
	ssh
set
	Status = 1 --(select dbo.udf_StatusValue('EDIToyota.ShipScheduleHeaders', 'Status', 'Active'))
from
	EDIToyota.ShipScheduleHeaders ssh
where
	ssh.Status = 0 --(select dbo.udf_StatusValue('EDIToyota.ShipScheduleHeaders', 'Status', 'New'))
